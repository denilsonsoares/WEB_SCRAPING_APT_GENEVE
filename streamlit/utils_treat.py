...
def tratar_dados(file_path, pasta_tratados):
    df = pd.read_excel(file_path)

    # Lista das colunas esperadas
    required_columns = ['Título', 'Preço (CHF)', 'Quartos', 'Espaço', 'Endereço', 'Link', 'Data']

    # Verificar se todas as colunas necessárias estão presentes
    if not all(column in df.columns for column in required_columns):
        print(f"O arquivo '{file_path}' está faltando colunas. Ignorando.")
        return
